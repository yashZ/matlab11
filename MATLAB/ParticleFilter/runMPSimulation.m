function rew = runMPSimulation(pomdp,realTrack,obsTrack,policy,VF,h)
    ...
    for t = 1:size(realTrack,2)
        ...
        elseif strcmp(policy,'IR')
            ...
            rew(t) = getIRReward(belief,a,pomdp,posOfPpl);
...
function rCov = getCoverageReward(belief,a,pomdp,posOfPpl)
    ...
    nPplReal = length(find(posOfPpl==1));
    nPplPred = length(find(PPos==1));
    if nPplReal == nPplPred 
        rCov = 1;
    else
        rCov = 0;
    end
...
function rIR = getIRReward(belief,a,pomdp,posOfPpl)
    ...
    %rIR = length(find((PPos - posOfPpl')==0));
