...
function actionVec = getActionEncodingP(nrGrids,nrPpl)
    ...
    actionVec1 = npermutek([1:nrGrids],nrPpl);
    actionVec2 = npermutek([nrGrids+1:2*nrGrids],nrPpl);
    actionVec = [actionVec1;actionVec2];
end
...
function r = getIndividualReward(fState,fAction,nrGrids)
...
function R = getSampleReward(nrstates,nrobs,nraction,encodedStates,encodedAction,encodedObs,nrGrids)
    ...
    for s = 1:nrstates
        for a = 1:nraction
            factoredState = encodedStates(s,:);
            factoredAction = encodedAction(a,:);
            r = getIndividualReward(factoredState,factoredAction,nrGrids);
            R(s,a) = r;
        end
    end
end
