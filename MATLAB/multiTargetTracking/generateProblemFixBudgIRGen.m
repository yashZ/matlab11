function sampleProb = generateProblemFixBudgIRGen()

% for one person only

nGrid = 4; nPpl = 1;budget = 3;
...
sampleProb.observation = getSampleObs(encodedStates,encodedObs,encodedAction,nGrid,budget);
sampleProb.transition = getSampleTrans(encodedStates,encodedAction,nGrid,budget);
...
function encodedAction = getEncodedAction(nGrid,encodedStates)
    ...
    %nAction = nGrid + 1;
    %encodedAction = npermutek([1:nAction],2);
    k = 0
    for i = 1:(nGrid+1)
...
function T = getSampleTrans(encodedStates,encodedAction,nGrid,budget)
...
function O = getSampleObs(encodedStates,encodedObs,encodedAction,nGrid,budget)
    ...
    for s = 1:nrstates
        for a = 1:nraction
            for o = 1:size(encodedObs,1)
                obs = encodedObs(o,:);
                if (s >= (nrstates - nGrid + 1)) && obs(1) == (nGrid+1)
                    O(s,a,o) = 0.25;
                end
            end
        end
...
%             if i==avar(2) && i < (nrstates - nrst + 1)
%                 R(i,j) = 1;
%             elseif i > (nrstates - nrst + 1
%             end
            ...
            elseif (avar(1) == (nrst+1)) && (i == avar(2))
                R(i,j) = 1;
            %elseif (avar(1) < (nrst+1)) && (i >= (nrstates - nrst + 1))
            %    R(i,j) = -1;
            end
        end
    end
