function S = sampleBeliefs(n, alg, maxStagnantCount, useEye, useSet)
% S = sampleBeliefs(n, alg, maxStagnantCount, useEye)
...
while size(S,1) <= n && iterCount < maxIterCount && stagnant < maxStagnantCount 
    ...
    switch alg
        case 'rand-dirichlet'
            newSet = addRandDirichlet(n);
        case 'rand-unif'
            newSet = addRandUnif(n);
        case 'pbvi-far'
            newSet = addPBVIFar(S);
        case 'pbvi-rand'
            newBels = addPBVIRand(Sorig); newSet = newBels;
            newBels = addPBVIRand(newBels); newSet = [newSet; newBels];
            newBels = addPBVIRand(newBels); newSet = [newSet; newBels];
            newBels = addPBVIRand(newBels); newSet = [newSet; newBels];
            newBels = addPBVIRand(newBels); newSet = [newSet; newBels];
            newBels = addPBVIRand(newBels); newSet = [newSet; newBels];
...
function dbl = addPBVIRand(S)
    ...
    for i = 1:problem.nrActions 
        ...
        %propSet(:,:,i) = (S*problem.transition(:,:,i))';
...
function bel = sampleObsAndUpdate( prevBel, action )
    ...
    prevBel = prevBel/sum(prevBel);
    
    obs = sampleObs( prevBel, action );
    %size(problem.observation(:,action,obs))
    %size(prevBel)
